		...
		if(checkPassword($_POST['password'], $errors))
		{
			...
			if(preg_match('#^(([a-z0-9!\#$%&\\\'*+/=?^_`{|}~-]+\.?)*[a-z0-9!\#$%&\\\'*+/=?^_`{|}~-]+)@(([a-z0-9-_]+\.?)*[a-z0-9-_]+)\.[a-z]{2,}$#i',$_POST['email']))
			{
				...
				if($dn == 0)
				{
					...
					if(mysqli_query($link, 'insert into users(id, username, password, email, signup_date, salt) values ('.$id.', "'.$username.'", "'.$password.'", "'.$email.'", "'.time().'","'.$salt.'")'))
					{
...
?>
		<div class="content">
			<form action="Register.php" method="post">
				...
				<strong><u>Please fill the following form to Register:</u></strong><br /><br>
				
				
				<div class="center" style="color:#363e40; font-size:1.1em">
					...
					<label for="passverif">Confirm Password</label><input type="password" name="passverif" /><br />
					<label for="email">Email</label><input type="text" name="email" value="<?php if(isset($_POST['email'])){echo htmlentities($_POST['email'], ENT_QUOTES, 'UTF-8');} ?>" /><br />
					
					<br><input style="color:blue" "font-size:1em" type="submit" value="ٌRegister" />
				</div>
			</form>
		</div>
<?php
